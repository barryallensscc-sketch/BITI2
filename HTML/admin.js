  ...
  if(isNew){
    ...
    s(K.settings,{
      ...
      slides: [
        ...
        { src: 'IMAGES/1.jpg', caption: '✦ Tech Excellence' }
      ]
    });
    s(K.submissions,[]);
  } else {
    var st = gO(K.settings);
    if (st && st.slides) {
      var hasImg1 = st.slides.some(function(x) { return x.src === 'IMAGES/1.jpg'; });
      if (!hasImg1) {
        st.slides.push({ src: 'IMAGES/1.jpg', caption: '✦ Tech Excellence' });
        s(K.settings, st);
      }
    }
  }

  if(!localStorage.getItem(K.courses) || !localStorage.getItem('biti_migrated_courses_v2')){
    s(K.courses,[
      {id:'c1',title:'Social Media Marketing & Content Moderation (SMCM)',description:'Learn digital campaign strategies, audience engagement, and platform policy moderation.',duration:'03 Months',icon:'💬'},
      {id:'c2',title:'Digital Business Operations (DBO)',description:'Master modern business workflows, CRM administration, project tools, and team coordination.',duration:'04 Months',icon:'⚙'},
      {id:'c3',title:'Advance Image Processing & Technique (AIPT)',description:'Study advanced editing, vector manipulation, digital restoration, and premium design methods.',duration:'04 Months',icon:'🎨'},
      {id:'c4',title:'AI Driven Freelancers (AIDF)',description:'Leverage cutting-edge generative AI models to accelerate client work and scale your global freelancing business.',duration:'03 Months',icon:'✦'}
    ]);
    localStorage.setItem('biti_migrated_courses_v2','true');
  }
...
function renderSubmissions(){
  ...
  var html='<div style="margin-bottom: 22px; display: flex; justify-content: flex-end;">' +
           '<button class="btn btn-primary" onclick="CMS.exportSubmissionsCSV()" style="display:inline-flex; align-items:center; gap:8px; background:var(--blue); color:#fff; border:0; padding:10px 16px; border-radius:4px; font-weight:700; cursor:pointer;">' +
           '<span>📥</span> Export General Leads to Excel (CSV)' +
           '</button>' +
           '</div>';
...
function exportSubmissionsCSV() {
  var items = g(K.submissions) || [];
  if (!items.length) { alert('No leads to export.'); return; }
  
  var csvRows = [];
  var headers = ["ID", "Date", "Name", "Email", "Phone", "Interested Course", "Message"];
  csvRows.push(headers.map(function(h){ return '"' + h.replace(/"/g, '""') + '"'; }).join(","));
  
  items.forEach(function(sub) {
    var dateStr = new Date(sub.createdAt || Date.now()).toLocaleString();
    var row = [
      sub.id || '',
      dateStr,
      sub.name || '',
  ...
  link.setAttribute("download", "Genaral Leads.csv");
