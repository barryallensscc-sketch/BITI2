...
document.addEventListener('DOMContentLoaded',function(){
  ...
  // Database Migration for Course list updates
  if(!localStorage.getItem('biti_migrated_courses_v2')){
    s(K.courses,[
...
function captureFormSubmissions(){
  ...
  forms.forEach(function(form){
    ...
    form.addEventListener('submit',function(e){
      ...
      // Sync to local server file if active
      fetch('http://localhost:3000/api/lead', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
      }).catch(function(err) {
        console.log('Local sync server offline. Data stored in browser storage.');
      });
...
function renderIndexSlideshow() {
  var settings = gO(K.settings);
  ...
  var slideIdx = 0;
  var slideImg = document.getElementById('hero-slideshow');
  var slideCap = document.getElementById('slideshow-caption');
  
  if (!slideImg || !slides.length) return;
  
  // Set initial slide
  slideImg.src = slides[0].src;
  if (slideCap) slideCap.textContent = slides[0].caption;
  ...
  setInterval(function() {
    ...
    setTimeout(function() {
      slideImg.src = slides[slideIdx].src;
      if (slideCap) slideCap.textContent = slides[slideIdx].caption;
